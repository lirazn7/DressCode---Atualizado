...
export default function ClosetScreen({ navigation }) {
  ...
  // NAVEGAÇÃO: 'home' | 'catalog' | 'pieces_home' | 'piece_detail'
  const [viewMode, setViewMode] = useState('home'); 
  
  // DADOS
  const [catalogs, setCatalogs] = useState([]); // Coleções/Looks
  const [pieceCategories, setPieceCategories] = useState([]); // Categorias/Estações
  const [selectedItem, setSelectedItem] = useState(null); // Pode ser um catálogo ou uma categoria de peça
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      // Busca Coleções (Looks)
      const { data: catData } = await supabase.from('catalogs').select('*').eq('userid', user.id).order('id', { ascending: false });
      setCatalogs(catData || []);

      // Busca Categorias de Peças (Estações/Ocasiões)
      const { data: pieceData } = await supabase.from('piece_categories').select('*').eq('userid', user.id).order('id', { ascending: true });
      setPieceCategories(pieceData || []);
  ...
  // ── CRIAR NOVO (COLEÇÃO OU CATEGORIA) ──
  const handleCreateNew = async () => {
    if (!newName.trim()) return;
    const table = modalTarget === 'collection' ? 'catalogs' : 'piece_categories';
    
    const { error } = await supabase.from(table).insert([{ userid: user.id, nome: newName, emoji: newEmoji }]);
    if (!error) {
      setNewName('');
  ...
  const openContent = async (item, type) => {
    ...
    const { data: items } = await supabase.from('catalog_items').select('*').eq(filterColumn, item.id).order('id', { ascending: false });
  ...
  const confirmUpload = async () => {
    ...
    setUploading(true);
    try {
      const base64Data = tempImage.base64;
      ...
      const dbPayload = {
        ...
        legenda: pieceLegenda,
        userid: user.id
      };

      if (viewMode === 'catalog') dbPayload.catalogid = selectedItem.id;
      else dbPayload.piece_category_id = selectedItem.id;

      const { error: dbError } = await supabase.from('catalog_items').insert([dbPayload]);

      if (!dbError) openContent(selectedItem, viewMode === 'catalog' ? 'collection' : 'piece_cat');
    } catch (error) {
      Alert.alert("Erro", "Falha ao salvar peça.");
    } finally { 
      setUploading(false); 
      setTempImage(null);
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {(viewMode === 'catalog' || viewMode === 'piece_detail') && (
        <FlatList
          data={itemsList}
          numColumns={2}
      ...
      {/* MODAL CRIAR (COLEÇÃO OU CATEGORIA) */}
